...
| 우선순위 | 건수 | 설명 |
|---------|------|------|
| **Must-have** | 6건 | 즉시 구현 필요 |
| **Should-have** | 7건 | MVP2, 다음 스프린트 |
| **Nice-to-have** | 2건 | MVP3 이후 |
| **Out-of-scope** | 16건 | 구현 불필요 또는 기존 기능으로 대체 |
...
| ~~Practice Quiz~~ | 성적 미반영, 즉시 정답 표시, 무제한 재시도 권장 | `quizMode: practice` 필드만 존재 | **Out-of-scope** | 현재 상태를 기본값으로 유지 |
| ~~Graded Survey~~ | 정답 없음, 완료 시 만점 자동 부여, Assignment 연동 | 없음 | **Out-of-scope** | |
| ~~Ungraded Survey~~ | 정답 없음, 성적 미반영, 익명 응답 가능 | 없음 | **Out-of-scope** | |
...
|------------|---------|----------|---------|
| ~~Access Code~~ | `accessCode` 필드 저장됨 | 응시 진입 시 코드 입력/검증 UI와 로직 없음 | **Out-of-scope** (프론트엔드 한계) |
| ~~IP Filter~~ | `ipRestriction` 필드 저장됨 | 서버사이드 IP 검증 불가 (프론트엔드 한계) | **Out-of-scope** (프론트엔드 한계) |
...
|----------|------------|---------|----------|
| **Short Answer** | 정답을 여러 개 등록 가능 (multiple correct answers). 학생 답이 그 중 하나와 일치하면 정답 | 단일 정답 + 유사도 기반 | **구현 필요** (Should-have, Effort 하) |
| **Matching** | Distractor(오답 보기) 추가 지원. 좌측보다 우측이 더 많을 수 있음 | 좌우 1:1 매칭만 | **구현 필요** (Should-have, Effort 중) |
| **Fill in Multiple Blanks** | 각 blank별로 복수 정답 등록 가능 | 단일 정답만 지원 (blank당 1개) | **구현 필요** (Should-have, Effort 하) |
| ~~Formula (Calculated)~~ | 변수 범위/소수점/허용오차 설정, 문항 생성 시 여러 세트 자동 생성 | 기본 수식 계산만 | **Out-of-scope** |
...
|------------|----------|---------|---------|--------|
| ~~Fudge Points~~ | 자동채점 결과에 교수자가 전체 가감점 적용 | 없음 | **Out-of-scope** | - |
| **문항별 코멘트 3종** | `correct_comments` / `incorrect_comments` / `neutral_comments`. 문항 생성 시 미리 설정 | 없음 | **Should-have** | 중 |
| ~~채점 완료 확정 흐름~~ | `pending_review` → `complete` 상태 전이, 최종 점수 산출 | 수동채점 저장만 됨 | **Out-of-scope** | - |
| ~~전체 코멘트 (Submission Comment)~~ | 학생 전체 제출물에 대한 종합 코멘트 | 학생별 코멘트 1개만 | **Out-of-scope** | - |
...
| Canvas 기능 | 동작 설명 | XN 현황 | 우선순위 | Effort |
|------------|----------|---------|---------|--------|
| ~~상태 자동 전이~~ | 시간 기반으로 draft→open→grading→closed 자동 전이 | draft→open만 구현 | **Out-of-scope** | - |
| **지각 제출 검증** | dueDate 경과 후 응시 시 차단 또는 지각 표시(late). `allowLateSubmit` 설정에 따라 동작 | 필드만 존재, 검증 로직 없음 | **Must-have** | 하 |
| **답변 자동 저장** (Autosave) | 응시 중 주기적으로(30초~2분) 현재 응답을 자동 저장. 브라우저 충돌 시 복구 가능 | 없음 (메모리 상태만) | **Should-have** | 중 |
| **예약 발행** | startDate 이전 접근 차단 + 퀴즈 리스트에 "예정" 배지 표시 | 필드만 존재, 접근 차단 없음 | **Should-have** | 하 |
| **마감 후 미제출자 일괄 자동 제출** | 마감 후 미제출 학생의 응시를 자동 제출 처리 | 타이머 만료 시 개별 auto-submit만 | **Should-have** | 중 |
| **Grace Period** | 제출 지연 허용 시간 (예: 마감 후 5분까지 허용) | 없음 | **Nice-to-have** | 하 |
...
| Canvas 기능 | 동작 설명 | XN 현황 | 우선순위 | Effort |
|------------|----------|---------|---------|--------|
| ~~복수 은행 동시 랜덤 출제~~ | Question Group에 여러 은행 연결, 은행별 N개 랜덤 추출 | 단일 은행만 | **Out-of-scope** | - |
| ~~Question Group 별 배점 차등~~ | Group 내 모든 문항에 동일 배점 지정 | 랜덤 출제에는 존재 | **기존 기능 개선** | 하 |
| ~~Account-level Bank~~ | 기관(학교) 전체에서 공유하는 문제은행 | 없음 | **Out-of-scope** | - |
| ~~Bank Bookmarking~~ | 다른 과목의 문제은행을 북마크하여 참조 | 없음 | **Out-of-scope** | - |
...
| # | 기능 | 현황 | Effort | 비고 |
|---|------|------|--------|------|
| 1 | **One Question at a Time** | 한 번에 하나의 문항만 표시 | 중 | 시험 모드 핵심 |
| 2 | **Lock Questions After Answering** | 답변 후 이전 문항 잠금 (#1 전제) | 하 | #1 구현 후 추가 |
| 3 | **hide_results** (학생 응답 조회 제어) | Always/Once/Until/Never 옵션 | 하 | |
| 4 | **one_time_results** (1회 조회) | 결과를 시도당 1회만 조회 | 하 | #3과 연동 |
| 5 | **Assign To** (차등 날짜) | 학생/그룹별 다른 Due Date / Available From / Until | 중 | 이전 구현 후 제거됨, 재구현 |
| 6 | **지각 제출 검증** | dueDate 경과 후 응시 차단 또는 지각 표시 | 하 | PM2 E-04 |

### Should-have (MVP2 -- 다음 스프린트, 7건)

| # | 기능 | 설명 | Effort |
|---|------|------|--------|
| 7 | **Fill in Multiple Blanks 복수 정답** | blank별 복수 정답 등록 (acceptedAnswers 패턴 적용) | 하 |
| 8 | **Short Answer 복수 정답** | 정답 여러 개 등록, 하나만 일치하면 정답 | 하 |
| 9 | **Matching Distractor** | 오답 보기 추가 지원 (우측이 좌측보다 많을 수 있음) | 중 |
| 10 | **문항별 코멘트 3종** | correct/incorrect/neutral 피드백 사전 설정 | 중 |
| 11 | **답변 자동 저장 (Autosave)** | 응시 중 주기적 자동 저장, 데이터 손실 방지 | 중 |
| 12 | **예약 발행** | 퀴즈 리스트에 "예정" 배지 추가, startDate 이전 접근 차단 | 하 |
| 13 | **마감 후 미제출자 일괄 자동 제출** | Outstanding submissions 처리 | 중 |

### Nice-to-have (MVP3+, 2건)

| # | 기능 | 설명 | Effort |
|---|------|------|--------|
| 14 | **Grace Period** | 제출 지연 허용 시간 (예: 마감 후 5분까지 허용) | 하 |
| 15 | **Quiz Log (활동 로그)** | 학생 응시 행동 추적, 부정행위 탐지 | 상 |

### Out-of-scope (구현 불필요 또는 기존 기능으로 대체)

| 기능 | 사유 |
|------|------|
| **Assignment Group** | Gradebook 연동 전제 기능. XN Quiz 독립 운영 범위에서 불필요 |
| **Missing Word** | Multiple Dropdowns 유형으로 완전 대체 가능 |
| **Moderate (전체)** | 조건부 재응시 + Assign To로 대체 |
| **Practice Quiz 전용 동작** | 현재 구현된 상태를 기본값으로 유지 |
| **Graded/Ungraded Survey** | 사용 빈도 낮음, 현 단계 불필요 |
| **접근 코드 검증** | 프론트엔드 한계, 서버 전환 시 재검토 |
| **IP Filter 검증** | 프론트엔드 한계, 서버 전환 시 재검토 |
| **채점 완료 확정 흐름** | 성적 메뉴에서 별도 처리 |
| **상태 자동 전이** | 예정 배지 추가로 대체 (예약 발행 항목) |
| **Fudge Points** | 현 단계 불필요 |
| **Formula 변수 세트 자동 생성** | 현 단계 불필요 |
| **전체 코멘트 (Submission Comment)** | 현재 학생별 코멘트 기능으로 충분 |
| **복수 은행 동시 랜덤 출제** | 현재 랜덤 출제 기능으로 대체 |
| **Account-level Bank** | 가져오기/내보내기 기능으로 대체 |
| **Bank Bookmarking** | 가져오기/내보내기 기능으로 대체 |
| **Bank 참조 공유 (cross-course)** | New Quizzes 전용 기능, Classic Quizzes에 해당 없음 (삭제) |
...
| 갭 분석 항목 | PM 보고서 기존 이슈 | 최종 결정 |
|-------------|-------------------|----------|
| ~~접근 코드 검증~~ | PM2 E-14 (Medium) | **Out-of-scope** (프론트엔드 한계) |
| 지각 제출 검증 | PM2 E-04 (High) | **Must-have 유지** |
| ~~채점 완료 확정~~ | PM2 E-08, PM5 High | **Out-of-scope** (성적 메뉴에서 처리) |
| ~~상태 자동 전이~~ | PM2 E-10 (High) | **Out-of-scope** (예정 배지로 대체) |
| 모바일 내비게이션 | PM1 Critical-1 | Canvas 갭 아닌 UX 이슈 (본 보고서 범위 외) |
| ~~Fudge Points~~ | 신규 발견 | **Out-of-scope** |
| 문항별 코멘트 3종 | 신규 발견 | **Should-have** |
| ~~Quiz Extension (Moderate)~~ | 신규 발견 | **Out-of-scope** (조건부 재응시 + Assign To로 대체) |
| One Question at a Time | 신규 발견 | **Must-have 확정** |
| Lock Questions After Answering | 신규 발견 | **Must-have 확정** |
| hide_results | 신규 발견 | **Must-have 확정** |
| one_time_results | 신규 발견 | **Must-have 확정** |
| Assign To (차등 날짜) | 이전 구현 후 제거됨 | **Must-have 확정** (재구현) |
| Autosave | 신규 발견 | **Should-have** |
| ~~Assignment Group~~ | - | **Out-of-scope 확정** |

**기획자 엑셀 검토 기반 우선순위 전면 재편**: 기존 Must-have 9건 중 3건(접근 코드 검증, 채점 완료 확정, 상태 자동 전이)이 Out-of-scope로 변경. 채점 Edge Case 3건(Short Answer/Matching/Fill in Multiple Blanks)과 Grace Period, Quiz Log가 신규 구현 대상으로 추가됨. 총 구현 대상 15건(Must-have 6 + Should-have 7 + Nice-to-have 2).
...
권고 구현 순서:
1. **One Question at a Time** (중) -- QuizAttempt 페이지에 단일 문항 표시 모드 추가, 문항 네비게이션 UI 필요
2. **Lock Questions After Answering** (하) -- #1 구현 후 "이전 문항 잠금" 옵션 추가
3. **hide_results + one_time_results** (하) -- 설정 필드 추가 + QuizAttempt 결과 화면 조건부 렌더링, 두 기능이 연동되므로 함께 구현
4. **Assign To (차등 날짜)** (중) -- 이전 구현 경험 있음, 재구현
5. **지각 제출 검증** (하) -- dueDate 경과 후 응시 차단/지각 표시

### MVP2 스프린트 (Should-have 7건, 권고 순서)
1. **Fill in Multiple Blanks 복수 정답** (하) -- Short Answer의 acceptedAnswers 패턴 적용
2. **Short Answer 복수 정답** (하) -- 기존 acceptedAnswers 배열 검증/확장
3. **Matching Distractor** (중) -- 오답 보기 추가, 우측 항목 확장
4. **문항별 코멘트 3종** (중) -- AddQuestionModal + QuizAttempt 결과 화면 수정
5. **예약 발행** (하) -- 퀴즈 리스트에 "예정" 배지 추가 + startDate 이전 접근 차단
6. **Autosave** (중) -- localStorage 기반 임시 저장, 서버 전환 시 API 교체
7. **마감 후 미제출자 일괄 자동 제출** (중) -- 마감 시점 미제출 응시 자동 처리
