...
```
교수자 (instructor):
사이드바 "문제은행" → /question-banks (SCR-I-BANK-LIST)
                       ├── 현재 과목 문제모음 카드 목록
                       ├── "새 문제모음" (이름 + 난이도 입력)
...
학생 (student):
사이드바 "문제은행" 진입점 미노출. 직접 URL 시 `<Navigate to="/" replace />`.

핵심 태스크 클릭 뎁스:
- 문제은행 진입: 사이드바 → 문제은행 (1단계)
- 문항 추가: 문제은행 → 모음 → 새 문항 (3단계)
- 일괄 업로드: 문제은행 → 모음 → 일괄 업로드 (3단계)
- 다른 과목에서 가져오기: 문제은행 → 모음 → 가져오기 → 출처 → 문항 선택 → 추가 (6단계)
```
