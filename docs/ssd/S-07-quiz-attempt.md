...
| **영역** | **설명** | **주요 컴포넌트** |
|---|---|---|
| 헤더 | 퀴즈 제목, 주차/차시/만점, 설명, 자동 저장 상태 | h1 |
| 우측 상단 | 남은 시간 카운트다운 (MM:SS) + 답변 완료 진행률 | Timer |
| 본문 | 문항 카드 (전체 표시 / 한 문항씩 표시) | QuestionCard |
| 네비게이션 | 이전/다음 버튼 또는 제출 버튼 | Button |
| 배너 | 자동 저장 실패 / 지각 제출 / 미리보기 모드 안내 | 색상별 배너 |
| 모달 | 제한시간 만료 / 응답 후 잠금 경고 / 미답변 스킵 경고 / 제출 결과 | AlertDialog, ConfirmDialog, ResultModal |
...
| **상태** | **트리거** | **표현** |
|---|---|---|
| 진입 차단 | lockDate/startDate/draft/grading/지각 마감 | 차단 화면 + "응시 시작 전" / "마감 후" 안내 |
