...
| 참조 코드 | `src/pages/QuizAttempt.jsx`, `src/components/PreflightGate.jsx` (SecurityActiveBadges 포함), `src/utils/randomGroups.js` (`expandRandomGroups`) |
...
응시 진입 시 시험 정의의 `random_group` placeholder 를 학생 단위 시드로 실제 문항 배열로 확장한다. 시드는 결정론적이라 새로고침/재접속 시에도 동일 학생은 동일 문항을 본다.

| **항목** | **내용** |
|---|---|
| 확장 함수 | `expandRandomGroups(rawItems, seedKey, getBankQuestions)` — `src/utils/randomGroups.js` |
| seedKey | 실응시: `${currentStudent.id}_${quizId}` / 미리보기: `preview_${currentStudent.id ?? 'anon'}_${quizId}` |
| 확장 시점 | `useMemo` 로 `rawItems / seedKey / getBankQuestions` 변경 시에만 재계산 (실응시 중 동일 학생은 매 렌더 동일 결과) |
| 학생별 결정론 | mulberry32 PRNG + Fisher-Yates 셔플 → 동일 (studentId, quizId, groupId) 조합은 항상 동일 결과 |
| 새로고침 / 재접속 | rawItems 와 seedKey 가 동일하므로 동일 문항 + 동일 순서 보장 |
| 응시 시점에 풀 문항 수 변경 (강사가 은행 수정) | `getBankQuestions(bankId)` 결과가 달라지면 확장 결과도 변할 수 있음 → 응시 시작 후 풀 변경은 운영상 권장하지 않음 (Phase 2 잠금 검토 — 간극 G-6) |
| 차등 배점 적용 | `random_group` placeholder 의 `difficultyPoints` 가 풀 문항 난이도에 매핑되어 학생별 합계가 달라질 수 있음. 학생 화면에는 문항별 적용 배점이 그대로 노출됨 |
...
| G-7 | random_group 응시 시작 후 풀 변경 잠금 정책 | (B) Phase 2 후속 | 강사가 응시 진행 중 문제모음을 수정하면 `expandRandomGroups` 결과가 달라질 수 있음. 응시 시작 시점의 풀 스냅샷 잠금 (`bankSnapshot`) + 학생별 expand 결과 캐시 필요 |
| G-8 | random_group placeholder 후속 새로고침 시 시드 누락 안내 | (B) C 분류 후속 카피 | 현재 동일 학생 + 동일 quizId 면 새로고침해도 동일 문항이지만, 학생 로그아웃 후 재로그인 등 edge case 검증 카피 필요 |
...
| **날짜** | **버전** | **변경 내용** | **변경자** |
|---|---|---|---|
| 2026-06-16 | v1.5 | 랜덤 출제 학생 응시 처리 추가. `expandRandomGroups(rawItems, seedKey, getBankQuestions)` 응시 진입 시 호출 + seedKey 규칙(실응시: `${studentId}_${quizId}` / 미리보기: `preview_*`) + 결정론 보장(새로고침/재접속 시 동일 문항) 명시. 차등 배점 학생별 합계 차이 안내. 간극 G-7 (풀 잠금 정책) / G-8 (시드 edge case) 추가. 참조 코드에 `randomGroups.js` 추가. | 김민주 (Creator/PD) |
