...
import { normalizeDate, shouldUseApi } from './_common'
import {
  mockQuizzes,
  addQuiz as mockAdd,
...
function toApiQuizBody(body) {
  const effectiveCode = body.courseCode || extractCourseCode(body.course)
  return {
    title: body.title,
...
export async function listQuizzes(params = {}) {
  ...
  if (params.courseCode) {
    const code = String(params.courseCode).toUpperCase()
...
/**
 * 문항 재채점 — option 별 자동채점 재계산 (교수자 수동채점 답안 포함 일괄 재채점)
...
export async function regradeQuestion(quizId, question, option, oldQuestion) {
  ...
  const { changedCount } = mockRegradeQuestion(quizId, question, option, oldQuestion)
  return {
