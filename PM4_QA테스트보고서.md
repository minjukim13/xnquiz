...
## 상시 체크리스트 — 숨은 블로커 (2026-04-24 추가)

**배경**: UX 감사에서 "사용자에게 피드백 없이 방치되는 비동기 액션" 이 반복 발견됨 (다운로드 진행 표시 누락 C3, 데이터 로드 실패 시 로딩 무한 유지 C6). 기능/성능 탭과 별도로 모든 QA 라운드마다 실행.

**체크 방법**: DevTools Network 패널 → Throttling `Slow 3G` → 액션 실행 후 3초 이상 시각 피드백 유무 확인

**게이트 등급**
- **Must-pass**: 실패 시 실데이터 연동 배포 차단 (사용자가 "멈춘 것으로 착각"하거나 데이터 손실 위험 직결)
- **Should-pass**: 실패해도 배포 가능하나 다음 스프린트 내 해결 필수 (UX 폴리싱 수준)

| ID | 액션 | 기대 피드백 | 게이트 |
|---|---|---|---|
| HB-01 | 엑셀 다운로드 (채점 대시보드) | 버튼 로딩 스피너 또는 토스트 "다운로드 준비 중" | **Must-pass** |
| HB-02 | 답안지 PDF 다운로드 (학생별) | 버튼 로딩 스피너 | **Must-pass** |
| HB-03 | 엑셀 일괄 채점 업로드 파싱 | 모달 내 진행 단계 + 실패 시 에러 영역 + 재시도 | **Must-pass** |
| HB-04 | 퀴즈 상세/채점 대시보드 초기 로드 실패 | 스켈레톤 → 에러 화면 + 재시도 버튼 (로딩 무한 금지) | **Must-pass** |
| HB-05 | 학생 응시 자동저장 실패 | 배너 + 재시도 버튼 (수동 복구 가능) | **Must-pass** (데이터 손실 직결) |
| HB-06 | 점수 일괄 저장 중 | 저장 버튼 비활성 + 진행 표시 (중복 클릭 방지) | **Must-pass** |
| HB-07 | 문제은행 가져오기/내보내기 | 마법사 단계별 로딩 + 실패 시 단계 내 에러 메시지 | Should-pass |
| HB-08 | 재채점 적용 후 대상 인원 확인 | 토스트 "N명 재채점 완료" + 재채점 로그 접근 링크 | Should-pass |

**정책**
- Must-pass 전부 통과해야 실데이터 연동 배포 가능
- Should-pass 실패 건은 [릴리즈 노트 Known Issues](XN_Quizzes_작업정리.md) 에 기재 + 다음 스프린트 Must-have 자동 편입
- 신규 비동기 액션 추가 시 이 표에 행 추가 + 게이트 등급 결정 의무
